...
const AdminMessageCenter: React.FC = () => {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.building !== 'ALL') {
        recipientIds = owners.filter(o => getBuilding(o.unit_number) === form.building).map(o => o.id);
      } else {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 mt-10 glass-card animate-fadeInUp">
      <h2 className="text-2xl font-bold text-white mb-6">Admin Message Center</h2>
      ...
      <form onSubmit={handleSearch} className="mb-6 flex gap-2">
        ...
        />
        <button type="submit" className="glass-button" disabled={searching}>{searching ? 'Searching...' : 'Search'}</button>
      </form>
      {searchResults.length > 0 && (
        <div className="mb-8 bg-white/5 rounded-lg p-4">
          <div className="font-semibold text-white mb-2">Search Results:</div>
          <ul className="space-y-2">
            {searchResults.map((r, i) => (
              <li key={i} className="border-b border-white/10 pb-2">
                ...
                <div className="text-xs text-white/40 mt-1">{r.metadata?.sentAt}</div>
              </li>
            ))}
          </ul>
        </div>
      ...
      <form onSubmit={handleSend} className="space-y-4">
        <div>
          <label className="block text-white font-medium mb-1">Message Type</label>
        ...
        <div>
          ...
          <input name="title" value={form.title} onChange={handleChange} className="glass-input" maxLength={200} required />
        </div>
        <div>
          <label className="block text-white font-medium mb-1">Body</label>
          <textarea name="body" value={form.body} onChange={handleChange} className="glass-input" rows={4} required />
        </div>
        <div>
          <label className="block text-white font-medium mb-1">Send To</label>
          <select name="building" value={form.building} onChange={handleChange} className="glass-input">
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center text-white">
            <input type="checkbox" name="urgent" checked={form.urgent} onChange={handleChange} className="mr-2" disabled={form.type === 'emergency'} />
            Mark as Urgent (future: triggers SMS/email)
          </label>
        </div>
        <div className="flex justify-end gap-4 mt-4">
          <button type="submit" className="glass-button" disabled={sending}>{sending ? 'Sending...' : 'Send Message'}</button>
        </div>
