    ...
    try {
      ...
      try {
        ...
        results.push({
          ...
          details:
            messages?.map((m) => `"${m.subject}" (${m.is_read ? 'read' : 'unread'})`).join(', ') ||
            'No messages',
        })
      } catch (error: any) {
        results.push({
        ...
        try {
          ...
          results.push({
            ...
            details:
              searchResults
                ?.slice(0, 3)
                .map((r: any) => `${r.first_name} ${r.last_name} (Unit ${r.unit_number})`)
                .join(', ') || 'No residents found',
          })
        } catch (error: any) {
          results.push({
            test: 'Resident Search',
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          {isAdmin && (
            <button
              ...
              className="flex items-center space-x-2 rounded-lg bg-blue-500/20 px-4 py-2 text-blue-300 transition-colors hover:bg-blue-500/30"
            >
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center space-x-2 rounded-lg bg-gradient-to-r from-blue-500 to-teal-600 px-6 py-2 text-white transition-colors hover:from-blue-400 hover:to-teal-500 disabled:opacity-50"
          >
        ...
        <div className="space-y-4">
          ...
          {testResults.map((result, index) => (
            <motion.div
              ...
              className={`glass-card border p-4 ${getStatusColor(result.status)}`}
            >
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="mb-2 text-white/90">{result.message}</p>
                  {result.details && <p className="mb-2 text-sm text-white/60">{result.details}</p>}
                  {result.fix && (
                    <div className="rounded border border-blue-500/20 bg-blue-500/10 p-2 text-sm text-blue-300">
      ...
      <div className="glass-card p-6">
        <h3 className="mb-4 text-lg font-semibold text-white">Setup Instructions</h3>
        <div className="space-y-4">
          <div className="flex items-start space-x-3">
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/70">
                Run <code className="rounded bg-white/10 px-2 py-1">./deploy-email-service.sh</code>{' '}
          ...
          <div className="flex items-start space-x-3">
            <Eye className="mt-0.5 h-5 w-5 flex-shrink-0 text-blue-400" />
            <div>
      ...
      {/* Feature Summary */}
      <div className="glass-card p-6">
        <h3 className="mb-4 text-lg font-semibold text-white">Messaging System Features</h3>
