...
const PhotoGalleryUpload: React.FC<PhotoGalleryUploadProps> = ({ onUploadComplete }) => {
  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow-md">
      <h3 className="mb-4 text-lg font-semibold text-gray-800">Upload to Gallery</h3>
      ...
      <div className="space-y-4">
        ...
        {uploadStatus === 'pending' && (
          <div className="rounded-md bg-yellow-50 p-3">
            <p className="text-sm text-yellow-800">⏳ Your photo is pending admin approval</p>
          </div>
        )}

        {uploadStatus === 'rejected' && (
          <div className="rounded-md bg-red-50 p-3">
            <p className="text-sm text-red-800">❌ Your photo was rejected: {rejectionReason}</p>
            <button
              onClick={() => {
                setUploadStatus('idle')
                setRejectionReason('')
